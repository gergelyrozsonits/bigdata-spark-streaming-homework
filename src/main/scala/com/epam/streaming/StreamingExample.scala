...
object StreamingExample {
  ...
  def updateFunc(values: Seq[Long], state: Option[Long]): Option[Long] = {
    val currentCount:Long = values.sum
    val previousCount:Long = state.getOrElse(0).asInstanceOf[Number].longValue()
  ...
  def task1(actorsOfMovies: InputDStream[String]): DStream[(String, Long)] = {
    actorsOfMovies
      .map(line => {
        val fragments:Array[String] = line.split("\t")
        val name:String = fragments(0)
        val title:String = fragments(1) + " " + fragments(2)
        (name, title)})
      .groupByKey()
      .map((tuple: (String, Iterable[String])) => (tuple._1, tuple._2.size.toLong))
      .transform((rdd: RDD[(String, Long)]) => rdd
        .map((tuple: (String, Long)) => (tuple._2, tuple._1))
        .sortByKey(true, 1)
        .map((tuple: (Long, String)) => (tuple._2, tuple._1)))
